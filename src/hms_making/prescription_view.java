...
public class prescription_view extends javax.swing.JFrame {
    ...
    public prescription_view() {
        initComponents();
         ...
         rs=db.getprescription();
        DefaultTableModel tb =(DefaultTableModel)prescriptiontable.getModel();
        
        try {
            while(rs.next()){
                String []prescriptiontable={rs.getString("id"), rs.getString("name"), rs.getString("med"), rs.getString("quantity"),rs.getString("dosge"),rs.getString("type"),rs.getString("gender")};
                //String []patientData=(rs.getString("P_Id"), rs.getString("P_Name"), rs.getString("P_Contact"), rs.getString("P_Gender"));
        tb.addRow(prescriptiontable);
            }//'"+id+"','"+name+"','"+med+"','"+quantity+"','"+dosge+"','"+type+"','"+gender+"'
        } catch (SQLException ex) {
            Logger.getLogger(prescription_view.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void initComponents() {
        ...
        prescriptiontable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Name", "Medicine", "Quantity", "Dosge", "Type", "Gender"
            }
        ));
        ...
        jLabel9.setText("PRESCRIPTION RECORD ");
...
int r=prescriptiontable.getSelectedRow();
String id=prescriptiontable.getValueAt(r, 0).toString();
...
prescription_view D=new prescription_view();
D.setVisible(true);
dispose();        // TODO add your handling code here:
